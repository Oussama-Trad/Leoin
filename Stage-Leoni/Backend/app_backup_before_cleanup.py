...
def get_user_news():
    ...
    try:
        ...
        print(f"🔍 GET_USER_NEWS: User - Department: {user_department}, Location: {user_location}", flush=True)
        
        # Construire le filtre pour les actualités
        news_filter = {
            '$or': [
                # Actualités globales (super admin)
                {'isGlobal': True},
                # Actualités ciblées pour le département ET la location de l'utilisateur
                {
                    '$and': [
                        {'targetDepartments': {'$in': [user_department]}},
                        {'targetLocations': {'$in': [user_location]}}
                    ]
                },
                # Actualités ouvertes à tous les départements de cette location
                {
                    '$and': [
                        {'targetDepartments': {'$in': ['all']}},
                        {'targetLocations': {'$in': [user_location]}}
                    ]
                }
            ],
        ...
        news = list(news_collection.find(news_filter).sort('createdAt', -1))
        
        print(f"🔍 GET_USER_NEWS: Trouvé {len(news)} actualités", flush=True)
        ...
        for article in news:
            formatted_news.append({
                ...
                'authorRole': article.get('authorRole', 'ADMIN'),
                'authorDepartment': article.get('authorDepartment', ''),
                'authorLocation': article.get('authorLocation', ''),
                'priority': article.get('priority', 'normal'),
                'isGlobal': article.get('isGlobal', False),
